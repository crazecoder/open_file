...
public class OpenFilePlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    func requestDiskPermission(onGranted:()->Void,onDenied:()->Void){
    ...
    func canOpen(fileURL: URL,completeHandler:@escaping (Bool)->Void) {
        guard let workspace = NSWorkspace.shared.urlForApplication(toOpen: fileURL) else {
            // 没有找到可以打开此文件的应用程序
            completeHandler(false)
            return
        }
        // 对于已选择的文件，可以尝试打开此文件，以验证工作区是否可以按预期方式打开此文件。
